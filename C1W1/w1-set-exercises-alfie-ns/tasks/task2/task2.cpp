...
int main(int argc, char* argv[])
{
    ...
    if (argc != 2) { // count of the command line arguments
        cerr << "Filename missing" << endl;
        return -1;
    }
    string strFile = argv[1]; // the file name

    //Read a vector of points from the file
    vector<Point> points = readDataPoints(strFile); // read the data points from the file
